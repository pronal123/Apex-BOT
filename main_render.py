# ====================================================================================
# Apex BOT v9.2.2-Apex - TOP30分散監視・超安定化版 (ULTRA STABILITY)
# 修正点: 
# 1. LOOP_INTERVALを180秒（3分）に延長。
# 2. SYMBOL_WAITを1.5秒に延長し、銘柄間遅延を大幅に強化。
# 3. これにより、TOP30を分析する負荷をAPIレート制限以下に抑制することを目指します。
# ====================================================================================
...
def format_telegram_message(signal: Dict) -> str:
    ...
    if signal['side'] == "Neutral":
        if signal.get('is_health_check', False):
            ...
            return (
                f"🚨 <b>Apex BOT v9.2.2-Apex - 死活監視 (システム正常)</b> 🟢\n"
...
async def send_test_message():
    """起動テスト通知"""
    test_text = (
        f"🤖 <b>Apex BOT v9.2.2-Apex - 起動テスト通知 (ULTRA STABILITY版)</b> 🚀\n\n"
        f"現在の時刻: {datetime.now(JST).strftime('%Y-%m-%d %H:%M:%S')} JST\n"
        f"<b>超安定化: 実行間隔を180秒、銘柄遅延を1.5秒に設定し、レート制限回避を最大化します。</b>"
    )
    try:
...
app = FastAPI(title="Apex BOT API", version="v9.2.2-Apex_ULTRA_STABILITY")

@app.on_event("startup")
async def startup_event():
    """アプリケーション起動時にCCXTクライアントを初期化し、メインループを開始する"""
    initialize_ccxt_client()
    logging.info("🚀 Apex BOT v9.2.2-Apex ULTRA_STABILITY Startup Complete.")
...
def get_status():
    ...
    status_msg = {
        "status": "ok",
        "bot_version": "v9.2.2-Apex_ULTRA_STABILITY",
...
def home_view():
    ...
    return JSONResponse(content={"message": "Apex BOT is running (v9.2.2-Apex_ULTRA_STABILITY)."}, status_code=200)
