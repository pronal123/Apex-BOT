...
async def initialize_exchange_client() -> bool:
    ...
    try:
        ...
        if client_name == 'binance':
            ...
            exchange_class = ccxt_async.mexc # MEXCクライアントを選択
        else:
...
async def main_bot_loop():
    ...
    if not IS_CLIENT_READY:
        ...
        # initialize_exchange_client()はstartupで実行されているため、ここでは状態を確認するのみ
        if not EXCHANGE_CLIENT or not IS_FIRST_MAIN_LOOP_COMPLETED:
            logging.critical("クライアント初期化失敗または未完了。次のループでリトライします。")
            return
