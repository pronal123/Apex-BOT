# ====================================================================================
# Apex BOT v17.0.0 - P&L Display Enhancement (v16.0.1 Base)
# - ENHANCEMENT 1: 1000ドルでのLimit注文を仮定し、TP/SL到達時の損益額（USD）と損益率を明確に表示。
# - ENHANCEMENT 2: Pivot S/Rレベル到達時の概算損益額を補助的に表示。
# ====================================================================================
...
DTS_RRR_DISPLAY = 5.0               
POSITION_CAPITAL = 1000.0           # 1x想定のポジションサイズ (USD) - $1000固定 (NEW)
...
def format_integrated_analysis_message(symbol: str, signals: List[Dict], rank: int) -> str:
    ...
    # 取引計画の表示をDTSに合わせて変更
    trade_plan = (
        f"**🎯 推奨取引計画 (Limit & Dynamic Trailing Stop)**\n"
        f"----------------------------------\n"
        f"| 指標 | 価格 (USD) | 備考 |\n"
        f"| :--- | :--- | :--- |\n"
    ...
    pnl_block = (
        f"\n**📈 損益結果 (${POSITION_CAPITAL:,.0f} Limit注文でTP/SLに到達時)**\n"
        f"----------------------------------\n"
    ...
    if pivot_r1 > 0 and entry_price > 0 and side in ["ロング", "ショート"]:
        ...
        # 損益ブロックの構築
        pivot_pnl_block = (
            f"\n**🧮 ${POSITION_CAPITAL:,.0f} ポジションの到達損益 (Limit Entry → Pivot S/R) [1x想定]**\n"
            f"----------------------------------\n"
    ...
    footer = (
        ...
        f"| ⚙️ **BOT Ver** | **v17.0.0** - P&L Display Enhancement |\n" # バージョン更新
        f"==================================\n"
...
async def main_loop():
    """
    メインの監視ループ - P&L Display Enhancement (v17.0.0)適用
    """
    ...
    while True:
        try:
            logging.info(f"\n--- 🚀 Apex BOT v17.0.0 - 監視ループ開始 (時刻: {datetime.now(JST).strftime('%Y-%m-%d %H:%M:%S')} JST) ---") # バージョン更新
            ...
            # エラー発生時のクライアント再初期化ロジックは、このファイル（v16.0.1ベース）には含まれていないため、一旦そのままにしておく
...
app = FastAPI(title="Apex BOT API", version="v17.0.0 - P&L Enhancement") # バージョン更新
    ...
    if EXCHANGE_CLIENT:
        try:
            ...
            logging.info("CCXTクライアントをシャットダウンしました。")
             ...
             # CCXTクライアントが既にクローズしている場合のエラーを無視
             pass 
...
def get_status():
    status_msg = {
        "status": "ok",
        "bot_version": "v17.0.0 - P&L Enhancement", # バージョン更新
...
@app.get("/")
def home_view():
    return JSONResponse(content={"message": "Apex BOT is running (v17.0.0)"})
