# ====================================================================================
# Apex BOT v12.1.16 - 超積極的なKC後方探索 (50 Bars)
# 
# 修正点:
# - v12.1.15で解消できなかったKCのNaN警告を解消するため、有効なKC値を見つけるための後方探索を
#   最新足から最大50本前の足まで大幅に拡張。データフレームの初期NaNゾーンを乗り越え、KCの堅牢性を最大化する。
# ====================================================================================
...
def format_integrated_analysis_message(symbol: str, signals: List[Dict], rank: int) -> str:
    ...
    footer = (
        ...
        f"| ⚙️ **BOT Ver** | v12.1.16 - Aggressive KC Lookback (50 Bars) |\n" # <-- バージョン変更
        f"==================================\n"
...
async def analyze_single_timeframe(symbol: str, timeframe: str, macro_context: Dict, client_name: str, four_hour_trend_context: str, long_term_penalty_applied: bool) -> Optional[Dict]:
    """
    単一の時間軸で分析とシグナル生成を行う関数 (v12.1.16: KC指標の後方探索を最大50本に拡張)
    """
    ...
    try:
        ...
        # =========================================================================
        # V12.1.16 NEW: KC指標値の抽出 (後方探索を最大50本に拡張)
        # =========================================================================
        ...
        if kc_upper_name in df.columns:
            # 最後に有効なKC値を見つける (最新の50バーをチェック)
            # i=1: 最新足, i=i: i-1本前の足
            for i in range(1, 51): # <-- 修正点: 後方探索を最大50本に拡張
        ...
        # kc_upper > 0 のチェックは、V12.1.16のデータ堅牢化ロジックによって保証される
        if kc_upper > 0: 
...
app = FastAPI(title="Apex BOT API", version="v12.1.16-AGGRESSIVE_KC_LOOKBACK (Full Integrated)") # <-- バージョン変更

@app.on_event("startup")
async def startup_event():
    logging.info("🚀 Apex BOT v12.1.16 Startup initializing...") # <-- バージョン変更
...
def get_status():
    status_msg = {
        "status": "ok",
        "bot_version": "v12.1.16-AGGRESSIVE_KC_LOOKBACK (Full Integrated)", # <-- バージョン変更
...
@app.get("/")
def home_view():
    return JSONResponse(content={"message": "Apex BOT is running (v12.1.16, Full Integrated, Aggressive KC Lookback)."}, status_code=200) # <-- バージョン変更
