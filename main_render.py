# ====================================================================================
# Apex BOT v19.0.31 - Dynamic SL/TP Applied (Patch 40)
#
# 改良・修正点:
# 1. 【機能追加】時間軸に基づきSL/TPを変動設定するダイナミックSL/TPロジックを統合。
# 2. 【ロジック変更】新しいTP/SLロジックに対応するため、シグナル適用前に calculate_dynamic_sl_tp を呼び出す。
# 3. 【バージョン更新】全てのバージョン情報を Patch 40 に更新。
# 4. 【FIX/致命的】format_usdt関数にNoneチェックを追加し、TypeErrorを回避。(元のv19.0.28から引き継ぎ)
# ====================================================================================
...
import json
import re
# import ftplib # FTPロジック削除 (元のv19.0.28から引き継ぎ)
import uuid 
...
# NOTE: MEXCに存在しない銘柄 (MATIC, FTM, EOS, MKR, THETA) はDEFAULT_SYMBOLSから除外しています。
DEFAULT_SYMBOLS = [
...
WEBSHARE_METHOD = os.getenv("WEBSHARE_METHOD", "HTTP") # デフォルトはHTTP
...
LAST_SIGNAL_TIME: Dict[str, float] = {} # クールダウンチェック用
...
# 取引ルール設定
TRADE_SIGNAL_COOLDOWN = 60 * 60 * 2 # シグナルクールダウン期間 (2時間)
...
# 💡 【新規】動的TP/SL設定 (時間軸ベース)
TIMEFRAME_RISK_PERCENTAGE: Dict[str, float] = {
    '1m':  0.005, # 0.5% のリスク幅
    '5m':  0.010, # 1.0% のリスク幅
    '15m': 0.015, # 1.5% のリスク幅 (元のコードのデフォルトに近い値)
    '1h':  0.025, # 2.5% のリスク幅
    '4h':  0.040, # 4.0% のリスク幅
}
INITIAL_RR_RATIO = 2.0        # リスクリワード比率 (シグナル生成時に使用)
...
def format_usdt(amount: float) -> str:
    """USDT金額を整形する (Noneチェックを追加)"""
    if amount is None: # FIX: Noneが渡された場合のTypeErrorを回避
        amount = 0.0
...
def get_score_breakdown(signal: Dict) -> str:
    ...
    # NOTE: この関数は分析ロジック (analyze_signals) の実装に合わせて調整される必要があります。
    # 簡易版のロジックから生成される tech_data を表示する想定です。
...
# ====================================================================================
# 🚀 新機能: Dynamic SL/TP ロジック
# ====================================================================================

def calculate_dynamic_sl_tp(signal: Dict[str, Any]) -> Dict[str, Any]:
    """
    シグナルオブジェクトに格納された時間軸とRRRに基づき、SL/TPの価格を変動設定する関数。
    """
    timeframe = signal.get('timeframe')
    entry_price = signal.get('entry_price')
    # RRRがシグナルに含まれていない場合、安全のためデフォルト値 2.0 を仮定
    rr_ratio = signal.get('rr_ratio', INITIAL_RR_RATIO) 
    
    if not all([timeframe, entry_price]):
        logging.warning("⚠️ Dynamic SL/TP: 必要な情報が不足しています。元のSL/TPを維持します。")
        return signal

    # 1. 時間軸に対応する目標リスク幅（パーセンテージ）を取得
    # 設定がない場合は、最も頻繁に使用される '15m' の値をデフォルトとする
    risk_percentage = TIMEFRAME_RISK_PERCENTAGE.get(timeframe, TIMEFRAME_RISK_PERCENTAGE['15m'])

    # 2. SL価格を計算 (ロングシグナルを想定: エントリー価格からリスク幅分下げる)
    sl_price = entry_price * (1.0 - risk_percentage)

    # 3. TP価格を計算 (TP幅 = SL幅 * RRR)
    tp_distance_percentage = risk_percentage * rr_ratio
    tp_price = entry_price * (1.0 + tp_distance_percentage)
    
    # 4. シグナル辞書を更新 (これが取引実行時に使われる)
    signal['stop_loss'] = sl_price
    signal['take_profit'] = tp_price
    signal['risk_percentage_used'] = risk_percentage # 通知用に記録

    logging.info(
        f"✅ Dynamic SL/TP適用: {signal['symbol']} ({timeframe}) - "
        f"リスク幅:{risk_percentage*100:.2f}%. RRR:{rr_ratio:.2f}. "
        f"新SL={sl_price:.4f}, 新TP={tp_price:.4f}"
    )

    return signal

# ====================================================================================
# UTILITIES & FORMATTING (続き)
# ====================================================================================
...
def format_analysis_only_message(all_signals: List[Dict], macro_context: Dict, current_threshold: float, monitoring_count: int) -> str:
    """1時間ごとの分析専用メッセージを作成する (元のロジックを維持)"""
    ...
    if sorted_signals:
        ...
        # SL/TPはダイナミックロジックが適用された後の値を使用
        initial_sl = top_signal['stop_loss']
        initial_tp = top_signal['take_profit']

        signal_section += (
            f"  🥇 <b>{symbol}</b> ({timeframe}) - **最高スコア** {score_color}\n"
            f"     - **総合スコア**: <code>{score * 100:.2f} / 100</code> (推定勝率: {get_estimated_win_rate(score)})\n"
            f"     - **初期RRR**: <code>{rr_display}</code>\n"
            f"     - **管理**: <code>時間軸連動ダイナミックSL/TP</code>\n"
            f"  \n**📊 スコア詳細ブレークダウン** (+/-要因)\n"
            f"  <code>- - - - - - - - - - - - - - - - - - - - -</code>\n"
            f"{breakdown_details}\n"
            f"  <code>- - - - - - - - - - - - - - - - - - - - -</code>\n"
    ...
    footer = (
        ...
        f"<i>Bot Ver: v19.0.31 - Dynamic SL/TP Applied (Patch 40)</i>" 
    )
...
def format_startup_message(
    ...
    """初回起動完了通知用のメッセージを作成する (元のロジックを維持)"""
    now_jst = datetime.now(JST).strftime("%Y/%m/%d %H:%M:%S")
    ...
    header = (
        ...
        f"  - **SL/TP管理**: <code>時間軸連動ダイナミックSL/TP</code>\n" # 修正
        f"<code>- - - - - - - - - - - - - - - - - - - - -</code>\n\n"
    )
...
def format_telegram_message(signal: Dict, context: str, current_threshold: float, trade_result: Optional[Dict] = None, exit_type: Optional[str] = None) -> str:
    ...
    breakdown_details = get_score_breakdown(signal) if context == "取引シグナル" else ""

    trade_section = ""
    trade_status_line = ""
    risk_percent_display = signal.get('risk_percentage_used', 0.015) * 100 if context == "取引シグナル" else 0.015 * 100 # 通知用に表示
    ...
    if context == "取引シグナル":
        ...
        exit_type_final = exit_type or '不明'
    ...
    message = (
        ...
        f"  - **ストップロス (SL)**: <code>{format_usdt(stop_loss)}</code> (リスク幅: {risk_percent_display:.2f}%)\n" # 修正
        f"  - **テイクプロフィット (TP)**: <code>{format_usdt(take_profit)}</code>\n"
        f"  - **管理ロジック**: <code>時間軸連動ダイナミックSL/TP</code>\n" # 修正
        f"<code>- - - - - - - - - - - - - - - - - - - - -</code>\n"
    )
    ...
    message += (f"<i>Bot Ver: v19.0.31 - Dynamic SL/TP Applied (Patch 40)</i>")
    return message


async def send_telegram_notification(message: str) -> bool:
    """Telegramにメッセージを送信する (元のロジックを維持)"""
...
def _to_json_compatible(obj):
    """
    再帰的にオブジェクトをJSON互換の型に変換するヘルパー関数。 (元のロジックを維持)
    """
...
def log_signal(data: Dict, log_type: str, trade_result: Optional[Dict] = None) -> None:
    """シグナルまたは取引結果をローカルファイルにログする (元のロジックを維持)"""
    try:
        log_entry = {
...
# ====================================================================================
# WEBSHARE FUNCTION (HTTP POST) (元のロジックを維持)
# ====================================================================================
...
async def initialize_exchange_client() -> bool:
    """CCXTクライアントを初期化し、市場情報をロードする (元のロジックを維持)"""
    global EXCHANGE_CLIENT, IS_CLIENT_READY
    
    IS_CLIENT_READY = False
    
    if not API_KEY or not SECRET_KEY:
         logging.critical("❌ CCXT初期化スキップ: APIキー または シークレットキー が設定されていません。")
         return False
...
async def fetch_account_status() -> Dict:
    """
    CCXTから口座の残高と、USDT以外の保有資産の情報を取得する。 (元のロジックを維持)
    """
...
async def adjust_order_amount(symbol: str, target_usdt_size: float) -> Optional[float]:
    """
    指定されたUSDT建ての目標取引サイズを、取引所の最小数量および数量精度に合わせて調整する。 (元のロジックを維持)
    """
    global EXCHANGE_CLIENT
...
def analyze_signals(df: pd.DataFrame, symbol: str, timeframe: str, macro_context: Dict) -> Optional[Dict]:
    ...
    if current_price > df['SMA200'].iloc[-1]:
        ...
        # 簡易的なRRR/SL/TP設定 (ダイナミックロジック適用前の**初期値**として使用)
        rr_ratio = INITIAL_RR_RATIO             
        # SL/TPの初期値は、最低限のSL幅(15mの0.015)で計算しておく
        initial_risk_percent = TIMEFRAME_RISK_PERCENTAGE['15m']
        stop_loss = current_price * (1 - initial_risk_percent)
        take_profit = current_price * (1 + initial_risk_percent * rr_ratio)
        ...
        if score > current_threshold and rr_ratio >= 1.0:
             return {
                ...
                'stop_loss': stop_loss, # 初期SL
                'take_profit': take_profit, # 初期TP
...
async def liquidate_position(position: Dict, exit_type: str, current_price: float) -> Optional[Dict]:
    ...
    try:
        ...
        trade_result = {
            ...
            'pnl_usdt': pnl_usdt,
            'pnl_rate': pnl_rate,
            'stop_loss': position['stop_loss'], # 最終のSL
            'take_profit': position['take_profit'], # 最終のTP
        }
        return trade_result
...
async def position_management_loop_async():
    """オープンポジションを監視し、SL/TPをチェックし決済する (元のロジックを維持)"""
    ...
    for position in positions_to_check:
        ...
        try:
            ...
            # --- 決済チェック (固定SL/TPのロジックを維持) ---
            if current_price <= sl:
                exit_type = "SL (ストップロス)"
            elif current_price >= tp:
                exit_type = "TP (テイクプロフィット)"
            
            # NOTE: ここではダイナミックSL/TPは取引時のみ適用され、監視は固定値で行われる。
            # トレーリングストップのようなロジックが必要な場合は、ここを改変する必要があるが、
            # 今回は時間軸に応じたSL/TP**設定**のみが要件であるため、固定値の監視を維持する。
...
async def execute_trade(signal: Dict) -> Optional[Dict]:
    """
    取引シグナルに基づき、取引所に対して注文を実行する。 (元のロジックを維持)
    """
...
async def main_bot_loop():
    """ボットのメイン実行ループ (1分ごと)"""
    global LAST_SUCCESS_TIME, IS_CLIENT_READY, CURRENT_MONITOR_SYMBOLS, LAST_ANALYSIS_SIGNALS, IS_FIRST_MAIN_LOOP_COMPLETED, GLOBAL_MACRO_CONTEXT, LAST_ANALYSIS_ONLY_NOTIFICATION_TIME, LAST_WEBSHARE_UPLOAD_TIME, LAST_SIGNAL_TIME, TRADE_SIGNAL_COOLDOWN
    ...
    for signal in LAST_ANALYSIS_SIGNALS:
        ...
        # 🚀 【追記】 TP/SLを時間軸に応じて変動させる
        signal = calculate_dynamic_sl_tp(signal)
        
        # 既存ポジションがないか、クールダウン期間を過ぎているかチェックしてから取引実行
        if (symbol not in LAST_SIGNAL_TIME or 
            (now - LAST_SIGNAL_TIME.get(symbol, 0.0)) >= TRADE_SIGNAL_COOLDOWN):
            ...
            if trade_result and trade_result.get('status') == 'ok':
                ...
                LAST_SIGNAL_TIME[symbol] = now # クールダウン時間を更新
        else:
            logging.info(f"スキップ: {symbol} はクールダウン期間中 ({TRADE_SIGNAL_COOLDOWN / 3600:.1f}h)。")
    ...
    if not IS_FIRST_MAIN_LOOP_COMPLETED:
        ...
        await send_telegram_notification(
            format_startup_message(
                ...
                len(CURRENT_MONITOR_SYMBOLS), 
                current_threshold,
                "v19.0.31 - Dynamic SL/TP Applied (Patch 40)" # バージョンを更新
            )
        )
        IS_FIRST_MAIN_LOOP_COMPLETED = True
        
    # 8. ログの外部アップロード (WebShare - HTTP POST)
...
def get_status_info():
    ...
    status_msg = {
        "status": "ok",
        "bot_version": "v19.0.31 - Dynamic SL/TP Applied (Patch 40)", # バージョンを更新
...
async def position_monitor_scheduler():
    """TP/SL監視ループを定期実行するスケジューラ (元のロジックを維持)"""
    while True:
        try:
            # position_management_loop_asyncは固定SL/TP監視ロジック
