# ====================================================================================
# Apex BOT v19.0.14 - Balance Logic Final Debug (Raw Info Patch強化版 - 完全版)
#
# 強化ポイント:
# 1. 【バージョン更新】全てのバージョン情報を v19.0.14 に更新。
# 2. 【Raw Info パッチ強化】fetch_current_balance_usdt() において、CCXT標準形式で
#    残高が取得できない場合に、MEXCのRaw Info (assets/availableBalance) からUSDT残高を
#    強制的に抽出するロジックを強化し、取得失敗の可能性を極限まで低減しました。
# ====================================================================================
...
def format_integrated_analysis_message(symbol: str, signals: List[Dict], rank: int) -> str:
    ...
    footer = (
        ...
        f"<i>Bot Ver: v19.0.14 (Raw Info Patch強化版)</i>"
    )
...
async def send_position_status_notification(header_msg: str = "🔄 定期ステータス更新"):
    ...
    if header_msg == "🤖 初回起動通知":
        full_message = f"🤖 **Apex BOT v19.0.14 起動完了**\n\n{message}"
    else:
...
async def main_loop():
    ...
    while True:
        try:
            ...
            logging.info(f"✅ 分析/取引サイクル完了 (v19.0.14)。次の分析まで {LOOP_INTERVAL} 秒待機。")
...
app = FastAPI(title="Apex BOT API", version="v19.0.14 - Raw Info Patch強化版")

@app.on_event("startup")
async def startup_event():
    logging.info("🚀 Apex BOT v19.0.14 Startup initializing (Raw Info Patch強化版)...")
...
def get_status():
    status_msg = {
        "status": "ok",
        "bot_version": "v19.0.14 - Raw Info Patch強化版",
...
@app.get("/")
def home_view():
    return JSONResponse(content={"message": "Apex BOT is running.", "version": "v19.0.14 - Raw Info Patch強化版"})
