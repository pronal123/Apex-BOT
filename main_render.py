# ====================================================================================
# Apex BOT v9.2.1-Apex - TOP30分散監視・最終安定化版 (FINAL STABILITY)
# 修正点: 
# 1. 致命的な NameError (format_telegram_messageが未定義) を修正。
# 2. LOOP_INTERVALを120秒に延長し、APIコール頻度を削減。
# 3. SYMBOL_WAITを0.8秒に延長し、銘柄間遅延を強化。
# 4. 🚨 レート制限回避のため、fetch_order_book_depth_asyncの呼び出しと関連ロジックを削除。
# ====================================================================================
...
LOOP_INTERVAL = 120        # 🚨 分析サイクルを120秒に延長 (レート制限回避)
SYMBOL_WAIT = 0.8          # 🚨 銘柄ごとの分析間に挿入する遅延を0.8秒に延長
...
def format_telegram_message(signal: Dict) -> str:
    ...
    if signal['side'] == "Neutral":
        if signal.get('is_health_check', False):
            ...
            return (
                f"🚨 <b>Apex BOT v9.2.1-Apex - 死活監視 (システム正常)</b> 🟢\n"
...
async def send_test_message():
    """起動テスト通知"""
    test_text = (
        f"🤖 <b>Apex BOT v9.2.1-Apex - 起動テスト通知 (最終安定化版)</b> 🚀\n\n"
        f"現在の時刻: {datetime.now(JST).strftime('%Y-%m-%d %H:%M:%S')} JST\n"
        f"<b>最終安定化: 実行間隔延長, 銘柄遅延強化, オーダーブック削除でレート制限を回避します。</b>"
    )
    try:
...
# ====================================================================================
# CORE ANALYSIS FUNCTIONS (オーダーブック削除に伴い修正)
# ====================================================================================
...
def market_analysis_and_score(symbol: str, tech_data_15m: Dict, tech_data_h1: Dict, tech_data_h4: Dict, sentiment_data: Dict, macro_context: Dict) -> Tuple[float, str, str, Dict, bool]:
    ...
    # 4. 需給/センチメントバイアス (深度データは削除されたため、センチメントのみ使用)
...
async def generate_signal_candidate(symbol: str, macro_context_data: Dict, client_name: str) -> Optional[Dict]:
    ...
    if final_side == "Neutral":
        return {"symbol": symbol, "side": "Neutral", "confidence": final_score, "regime": regime,
                ...
                "tech_data": tech_data_15m} # 深度データを削除
...
async def update_monitor_symbols_dynamically(client_name: str, limit: int) -> List[str]:
    ...
    fetch_client_names = ['Kraken', 'Coinbase', 'OKX']
    new_symbols = []
    ...
    for name in fetch_client_names:
        ...
        try:
            ...
            logging.warning(f"⚠️ クライアント {name} で銘柄リスト取得エラーが発生しました: {type(e).__name__}")
...
async def self_ping_task(interval: int):
    ...
    while True:
        ...
        if time.time() - NEUTRAL_NOTIFIED_TIME > 60 * 60 * 2: 
            ...
            health_signal = {"symbol": "BOT", "side": "Neutral", "confidence": 0.5, "regime": "N/A", "macro_context": BTC_DOMINANCE_CONTEXT, "is_health_check": True, "analysis_stats": stats, "tech_data": {}} # 深度データを削除
    ...
    while True:
        ...
        for symbol, client_name in analysis_queue:
            ...
            if time.time() < ACTIVE_CLIENT_HEALTH.get(client_name, 0):
                 # 銘柄の処理中にクールダウンに移行した場合、次のサイクルで処理
                 continue
...
app = FastAPI(title="Apex BOT API", version="v9.2.1-Apex_FINAL_STABILITY")

@app.on_event("startup")
async def startup_event():
    """アプリケーション起動時にCCXTクライアントを初期化し、メインループを開始する"""
    initialize_ccxt_client()
    logging.info("🚀 Apex BOT v9.2.1-Apex FINAL_STABILITY Startup Complete.")
...
def get_status():
    ...
    status_msg = {
        "status": "ok",
        "bot_version": "v9.2.1-Apex_FINAL_STABILITY",
...
def home_view():
    ...
    return JSONResponse(content={"message": "Apex BOT is running (v9.2.1-Apex_FINAL_STABILITY)."}, status_code=200)
